...
| dbt-bigquery | 1.11.x | Data transformation |
...
```bash
git clone https://github.com/noopsd123/amazon-price-analytics.git
cd amazon-price-analytics
```
...
python load/load.py
cd transform/ecommerce_dbt && dbt run && dbt test
```
...
# Load to BigQuery only
python load/load.py

# Transform with dbt (from transform/ecommerce_dbt/)
dbt run --select fct_weekly_snapshots
dbt run --select dim_products
dbt test

# Run full pipeline via Prefect
prefect deployment run 'amazon-pipeline/main'
```
...
📧 [LinkedIn](https://www.linkedin.com/in/noopurdivekar)  
